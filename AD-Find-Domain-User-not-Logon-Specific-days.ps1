...
try {
    $days = Read-Host -Prompt "Enter the number of days for user inactivity"
...
# Find Inactive Users
try {
    $inactiveUsers = Get-ADUser -Filter {LastLogonTimeStamp -lt $dateThreshold -and Enabled -eq $true} -Properties DisplayName, SamAccountName, Title, Office, LastLogonTimeStamp
    $results = @()

    foreach ($user in $inactiveUsers) {
        $lastLogonDate = [datetime]::FromFileTime($user.LastLogonTimeStamp)
        $output = "User: $($user.SamAccountName) - Display Name: $($user.DisplayName), Last Logon: $($lastLogonDate)"
        Write-Host $output
